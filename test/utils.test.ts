...
describe("utils", () => {
  ...
  it("normalize addresses to checksum, given eth_mainnet chainId", () => {
    ...
    ethMainnet.forEach((testAddress) => {
      expect(toChecksumAddress(testAddress.toLowerCase(), chainId)).toEqual(
        testAddress
      );
    });
  });

  it("normalize addresses to checksum, given rsk_mainnet chainId", () => {
    const chainId = 30;
    const rskMainnet = [
      "0x6549F4939460DE12611948B3F82B88C3C8975323",
      "0x27b1FdB04752BBc536007A920D24ACB045561c26",
    ...
    rskMainnet.forEach((testAddress) => {
      expect(toChecksumAddress(testAddress.toLowerCase(), chainId)).toEqual(
        testAddress
      );
    });
  ...
  it("normalize addresses to checksum, given rsk_testnet chainId", () => {
    const chainId = 31;
    const rskTestnet = [
      "0x42712D45473476B98452F434E72461577D686318",
      "0x27B1FdB04752BbC536007a920D24acB045561C26",
    ...
    rskTestnet.forEach((testAddress) => {
      expect(toChecksumAddress(testAddress.toLowerCase(), chainId)).toEqual(
        testAddress
      );
    });
